...
class Agent:
    def __init__(self):
        ...
        for s in states:
            ...
            self.N_Q[s] = [0,
                           0]  # First element is the number of visits of "Hit" at state s, second element is the Q value of "Stand" at s
    ...
    def simulate_till_termination(self, s, policy):
        trajectory = []
        curr = s
        while curr:
            trajectory.append(curr)
            a = policy(curr)
            curr = self.make_one_transition(a)
        return trajectory
    ...
    def MC_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            # calculate the trajectory from current state to terminal state
            trajectory = self.simulate_till_termination(self.simulator.state, self.default_policy)
            G = {}  # Directory: store the to-go value of each state in the trajectory
            ns = None  # next state to the current state
            # calculating to-go value backward (from terminal to initial)
            # G[s] = R(s0) + γ*R(s1) + ... + γ^i*R(si)
            for s in reversed(trajectory):
                next_g = DISCOUNT * G[ns] if ns else 0
                G[s] = self.reward(s) + next_g
                ns = s  # update this state as the next (since looping backward)
                self.S_MC[s] += G[s]  # update sum of return
                self.N_MC[s] += 1  # update number of sample
                self.MC_values[s] = self.S_MC[s] / self.N_MC[s]  # update MC value = avg(G)
    ...
    def TD_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            while s:
                ...
                next_v = DISCOUNT * self.TD_values[next_s] if next_s else 0
                self.TD_values[s] = self.TD_values[s] + self.alpha(self.N_TD[s]) * \
                                    (self.reward(s) + next_v - self.TD_values[s])
                self.N_TD[s] += 1  # update visit count
                s = next_s
    ...
    def Q_run(self, num_simulation, tester=False, epsilon=0.4):
        ...
        for simulation in range(num_simulation):
            ...
            while s:
                ...
                next_a = self.Q_values[next_s][0] < self.Q_values[next_s][1] if next_s else 0
                next_q = DISCOUNT * self.Q_values[next_s][next_a] if next_s else 0
                self.Q_values[s][a] = self.Q_values[s][a] + self.alpha(self.N_Q[s][a]) * (self.reward(s) + next_q - self.Q_values[s][a])
                self.N_Q[s][a] += 1
                s = next_s
    ...
    def pick_action(self, s, epsilon):
        if random.random() < epsilon:
            ...
            return self.Q_values[s][0] < self.Q_values[s][1]
